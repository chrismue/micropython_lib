...
if 0 == sel:
    ...
    tile.fill_rgb(255,255,255)
    while True:
        acc=lsm9d1.accel.xyz()
        #sense.measure()
        #while not sense.is_ready():
        #    machine.idle()
        r=int(acc[1]*512) if acc[1]>0 else 0
        g=int(-acc[1]*512) if acc[1]<0 else 0
        tile.illu(r, g, 0)
        #mag = " mag = (" +(", ".join(["%+6.3f"%i for i in lsm9d1.mag.xyz()])) + ") G" 
        #print(mag)
        
        # tile.set_dot(radar_x, radar_y, 255, 255, 255)
        # tile.brightness(int(acc[1]/10))
        # print("illu")
        # print(sense.lux())
